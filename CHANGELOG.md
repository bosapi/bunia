...
- Double-^C required to exit `bun run dev` and `bun run start`. Two distinct defects converged into the same symptom. (1) `core/server.ts` shutdown called `app.stop()` (no arg), which leaves Bun waiting on _idle_ HTTP/1.1 keep-alive sockets — browsers hold these for minutes — so even after `✅ All requests drained` printed (`inFlight === 0`), the server hung until the 5s force-exit `setTimeout` fired. Users press ^C again rather than wait. Fixed by passing `closeActiveConnections=true` to `app.stop(true)` so Bun severs idle sockets immediately, and tightened the safety-net `setTimeout` from 5s → 2s. (2) `core/dev.ts` had no `SIGINT`/`SIGTERM` handler, so the terminal's ^C reached the parent _and_ child app process simultaneously — the parent dev process exited instantly (default behavior), the child blocked in the now-stuck `app.stop()`, and the user had to ^C again to kill the orphaned child. Dev now installs handlers that flip `intentionalKill = true`, send `SIGTERM` to the spawned `appProcess`, race `appProcess.exited` against a 2.5s timeout, then `process.exit(0)`. A second ^C during shutdown calls `process.exit(130)` so impatient users still get a fast exit.
- SSR streaming render errors and slow-client backpressure in `core/renderer.ts` (`renderSSRStream`). Two prod-readiness defects fixed together because both stem from work happening inside the `ReadableStream` after headers commit. (1) **Render-error recovery**: `render(App, …)` now runs _before_ the stream is constructed. On throw, the response is `renderErrorPage(500, "Internal Server Error", url, req)` — proper status code, fully-styled `+error.svelte` when present (plain-text fallback otherwise) — instead of streaming a bare `<p>Internal Server Error</p></body></html>` after a 200 shell + spinner had already been flushed. (2) **Backpressure + cancellation**: replaced the eager `start(controller)` that enqueued shell/metadata/tail up front with a `pull(controller)`-based state machine over a pre-computed chunks array. Bun only invokes `pull` when its internal queue has room, so chunks no longer pile up in memory for slow clients. A `cancel()` handler plus `req.signal` abort listener (`{ once: true }`, removed in both terminal paths) short-circuits subsequent pulls when the client TCP connection drops or the consumer tears down the stream — no more wasted work after disconnect. The `ssr=false` short-circuit was hoisted out of the stream into a single non-streaming `Response` (shell + metadata + empty-body tail concatenated), since there is nothing to render and the chunks are tiny. TTFB cost is the synchronous `render()` time (typically <10ms); accepted trade-off for prod safety. Pre-stream phases (metadata load, parallel `loadRouteData` + module imports, `HttpError`/`Redirect` handling) untouched.
- Prerender process cleanup in `core/prerender.ts`. Three defects addressed: (1) hardcoded port `13572` replaced with OS-assigned ephemeral port via new exported `getEphemeralPort()` helper (binds `127.0.0.1:0`, captures the assigned port, closes the listener) — eliminates non-deterministic build failures from port collisions and unblocks concurrent builds; (2) child kill is now verified — spawn lifecycle wrapped in `try/finally` so `child.kill(); await child.exited` always runs, on success, error, and signal paths (the inline `child.kill()` in the not-ready branch was removed to avoid double-kill); (3) `SIGINT`/`SIGTERM` handlers installed via `process.once` for the duration of prerender — on receipt, the spawned server is killed and reaped before the parent re-raises the signal to itself (`process.kill(process.pid, sig)`) so exit code matches the standard `128+signum` convention. Handlers are removed in `finally` so prerender does not permanently override Node's default signal handling. New unit tests for `getEphemeralPort()` (returns a port in ephemeral range, returns different ports across calls).
...
- Unit tests for build/codegen helpers: `scanner` (page/api discovery, route groups, layout chain, layout/page `trailingSlash` cascade, `+error.svelte` detection, dotfile + `node_modules` skip), `routeTypes` (per-pattern `Params` extraction for `[slug]` / `[...rest]` / multi-dynamic / route-group exclusion, `PageServerLoad` / `Action` / `ActionData` emission gated on `+page.server.ts` presence, `LayoutServerLoad` emission, `ensureRootDirs` add/dedupe/preserve/parse-error paths), `envCodegen` (server output: `PUBLIC_STATIC_*` inline + `PUBLIC_*` / `STATIC_*` / private dynamic via `process.env`; client output: only `PUBLIC_*` exported, dynamic via `window.__BOSIA_ENV__`; types: `declare module '$env'` with all keys), `hooks.sequence` (composition order — outer wraps inner, short-circuit, `event.locals` mutation visibility), `paths.resolveBosiaBin` (resolution + nested-fallback contract), `lib/utils.cn` (clsx flattening + twMerge conflict resolution) and `getServerTime`, `cli/registry.mergePkgJson` (deps / devDeps / scripts add, no-overwrite, no-write-when-unchanged) and `resolveLocalRegistry`, and `prerender` pure helpers (`substituteParams`, `canonicalRouteFor`, `prerenderOutPath`, `prerenderDataPath`). 77 new tests across 8 files (167 total).

### Fixed

- `scanner` now reads `trailingSlash` from `+page.server.ts` / `+layout.server.ts` correctly. Previously `readTrailingSlash()` was passed a `src/routes/`-relative path but called `readFileSync()` (cwd-relative), so the file was never found and the `try/catch` silently fell back to `"never"` regardless of the route's actual setting. Fixed by joining `ROUTES_DIR` to the path at the call sites.
